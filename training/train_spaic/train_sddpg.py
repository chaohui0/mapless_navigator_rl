...
sys.path.append('../../')
from training.train_spaic.sddpg_agent import AgentSpiking
...
def train_sddpg(run_name="SNN_R1", exp_name="Rand_R1", episode_num=(100000, 1600, 1700, 1800),
    ...
    while True:
        ...
        spike_state_value = ddpg_state_2_spike_value_state(state, spike_state_num)  # list(np.array(6), np.array(5x360))
        ...
        dwa_flag = 0
        episode_reward = 0
        h_0 = torch.zeros(batch, hidden_size).to(device)
        for ita in range(ita_per_episode):  # 5000
            ...
            # print('h_0 size: ', h_0.shape)
            # print(spike_state_value)
            raw_action, raw_snn_action, h_0 = agent.act(spike_state_value, h_0,ita=ita)
        ...
        if overall_episode % 50 == 0:
            # max_w, min_w, max_b, min_b, shape_w, shape_b = agent.save("../save_sddpg1_weights",
            #                                                           0, run_name)
            agent.save("../save_weights_real_GRU_original", overall_episode, run_name)
