...
def network_2_robot_action_decoder(action, l_max, l_min, a_max, a_min):
    ...
    linear = action[0] * (l_max - l_min) + l_min
    angular = action[1] * (a_max - a_min) + a_min
...
def wheeled_dwa_encoder(action, wheel_max, wheel_min, diff=0.125):
    """
    Encode DWA output to actor output
    :param action: list or array
    """
    a = action[0] - diff * action[1]
    b = action[0] + diff * action[1]
    snn_action_l = (a - wheel_min) / (wheel_max - wheel_min)
    snn_action_a = (b - wheel_min) / (wheel_max - wheel_min)
    return [snn_action_l, snn_action_a]

def compute_current_achivable_vel(action, linear_acc, angular_acc, dt):
    """
    Compute the max, min velocity according to current velocity
    """
    lin_max = action[0] + linear_acc * dt
    lin_min = action[0] - linear_acc * dt
    ang_max = action[1] + angular_acc * dt
    ang_min = action[1] - angular_acc * dt

    return lin_min, lin_max, ang_min, ang_max
...
def ddpg_state_2_spike_value_state(state, spike_state_num=366,
    ...
    """
    # state: np.array(364)
    normal_state = state[0]   # np.array(4)
    overall_spikes_value = []
    spike_state_value = [0 for _ in range(6)]
    if normal_state[0] > 0:
        spike_state_value[0] = normal_state[0] / goal_dir_range
        spike_state_value[1] = 0
    else:
        spike_state_value[0] = 0
        spike_state_value[1] = abs(normal_state[0]) / goal_dir_range
    spike_state_value[2] = normal_state[1]
    spike_state_value[3] = normal_state[2] / linear_spd_range
    if normal_state[3] > 0:
        spike_state_value[4] = normal_state[3] / angular_spd_range
        spike_state_value[5] = 0
    else:
        spike_state_value[4] = 0
        spike_state_value[5] = abs(normal_state[3]) / angular_spd_range
    overall_spikes_value.append(np.array(spike_state_value))
    overall_spikes_value.append(state[1])
    ...
    return overall_spikes_value  # list(np.array(6), np.array(1x360))


def ddpg_state_rescale(state, normal_state_num,
                       goal_dir_range=math.pi, linear_spd_range=0.5, angular_spd_range=2.0):
    """
    state: [list(1x4), np.array(1x480x640)]  : list(1x4)-> [Direction to goal, Distance to goal, sqrt(linear.x**2 + linear.y**2), msg.twist[-1].angular.z]

    Rescale DDPG states to make them same scale with SNN input
    :param state: single ddpg state    (1 x 22)
    :param state_num: number of states (22)
    :param goal_dir_range: range of goal dir
    :param linear_spd_range: range of linear spd
    :param angular_spd_range: range of angular spd
    :return: rescale_state_value
    """
    # state: [dir_to_goal, dis_to_goal, sqrt(linear.x**2+linear.y**2), angular.z, 18*scan_data] : 1 x 22
    tmp_list = []
    normal_state = state[0]                                       # np.array(4)
    scan_state = state[1]                                         # np.array(5x360)
    rescale_state_value = np.zeros(normal_state_num)              # [0] x 4
    if normal_state[0] > 0:
        rescale_state_value[0] = normal_state[0] / goal_dir_range
    else:
        rescale_state_value[0] = -abs(normal_state[0]) / goal_dir_range
    rescale_state_value[1] = normal_state[1]
    rescale_state_value[2] = normal_state[2] / linear_spd_range
    if normal_state[3] > 0:
        rescale_state_value[3] = normal_state[3] / angular_spd_range
    else:
        rescale_state_value[3] = -abs(normal_state[3]) / angular_spd_range

    tmp_list.append(rescale_state_value)   # rescaled normal state: np.array(4)
    tmp_list.append(scan_state)            # scan state           : np.array(5x360)
    return tmp_list
