...
import sys
sys.path.append('../../')
from training.train_spiking_ddpg_original.sddpg_networks import ActorNetSpiking, CriticNetSpiking
...
class AgentSpiking:
    ...
    def __init__(self,
        ...
        self.save_name_actor  = '../save_sddpg_weights/SNN_R1SNN_actor_weights_246.pth'
        self.save_name_critic = '../save_sddpg_weights/SNN_R1SNN_critic_weights_246.pth'
        '''
        Random Action
        '''
        ...
        if self.use_trained:
            ...
            self.critic_net = CriticNetSpiking(self.state_num, self.action_num, self.device,
            ...
            self.target_critic_net = CriticNetSpiking(self.state_num, self.action_num, self.device,
                                                        hidden1=critic_net_dim[0],
                                                        hidden2=critic_net_dim[1],
                                                        hidden3=critic_net_dim[2],
                                                        batch_window=self.batch_window)
            ...
            model_data_actor = torch.load(self.save_name_actor)
            model_data_critic = torch.load(self.save_name_critic)

            self.actor_net.load_state_dict(model_data_actor['model_dict'])
            self.actor_optimizer.load_state_dict(model_data_actor['optimizer_dict'])
            ...
            self.target_actor_net.load_state_dict(model_data_actor['model_dict'])
        ...
        # print('actor: ', next(self.actor_net.parameters()).device)
        # print('critic: ', next(self.critic_net.parameters()).device)
        # print('target actor: ', next(self.target_actor_net.parameters()).device)
        # print('target critic: ', next(self.target_critic_net.parameters()).device)
        """
    ...
    def act(self, state, explore=True, train=True):  # state:  list(np.array(6), np.array(1x360))
        """
        ...
        # scan_state   = state[1]   # np.array(1x360)
