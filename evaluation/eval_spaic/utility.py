...
from training.train_spaic.spaic_networks import ActorNetSpiking
...
def load_spaic_actor_network(direct, filename, device, batch_window=50,  state_num=366, action_num=2):
    actor_net = ActorNetSpiking()
    actor_net.build()
    actor_net.state_from_dict(filename=filename, direct=direct, device=device)
    return actor_net
    

def load_test_actor_snn_network(model_dir, device, batch_window=50,
                                state_num=366, action_num=2, dim=(256, 256, 256)):
    """
    Load actor snn network for testing
    :param weight_dir: directory of numpy weights
    :param bias_dir: directory of numpy bias
    ...
    :return: actor_net
    """
    actor_data = torch.load(model_dir)
    actor_net = ActorNetSpiking(state_num, action_num, device,
                                batch_window=batch_window,
                                hidden1=dim[0],
                                hidden2=dim[1],
    ...
    actor_net.load_state_dict(actor_data['model_dict'])
